...
export default function FilmWidthCal() {
  ...
  const areaCal = () => {
    const areaAmout = length*(width/1000);
    return isNaN(areaAmout) ? 0 : areaAmout;
  };

  const calculateInk = () => {
    const areaAmout = areaCal();
    const inkAmount = ((areaAmout)*(coverage/100)*((100+waste)/100)*bmc*(transfer/100))/1000;
    ...
    <>
      <form>
        <div className=" px-2 md:px-20 mt-6">
          <h1 className="text-lg mt-4 text-gray-900">Film Width</h1>
